...
\documentclass[10pt, a4paper, twoside, openright]{custard}
...
% Abstract comes before the contents page.	
	\begin{abstract}
		...
		Machine learning gets perceived as a black box and something that will take over the world. Creating machines that will over through the human race, however, the concept of machine learning is a lot more narrowly focused in real life than what gets depicted in sci-fi movies, which is also not a black box. It is more of a collection of tools that can be used, based on the required task. While machine learning can be tricky to master, the underlying concepts are quite simple but very elegant in their approach. That is why we have proposed a game that allows the player to interact with different key machine learning algorithms and models while providing mediums to help educate and teach the players the understanding of the machine learning and provide knowledge on how they operate. We have achieved this by creating learning research to accompany the game. While at the centre of it all, having a fun and engaging game, that uses machine learning to teach users about machine learning. Through using Python, PyQT5 and industry-standard accepted libraries and packages, like Tensorflow and Sci-kit Learn. The game will provide key gameplay features that users would expect of games, which will be achieved by using fundamental gamification techniques, to create a fun and engaging game that allows the user to interact directly with the machine learning models.
		
		The code to this thesis project can be found here:
		\begin{center}
			\url{https://github.com/codingWithAndy/Thesis_Project}
		\end{center}
...
% Insert the code for each of your chapters
	...
	\input{./chapter/background_lit_review}
